    ...
    loop {
        if let Ok(atqa) = rfid.reqa() {
            if let Ok(uid) = rfid.select(&atqa) {
                ...
                rfid.hlta().expect("failed to enter into HALT state");
                rfid.stop_crypto1()
                    .expect("failed to switch off the MIFARE Crypto1 unit.");
...
pub static PICOTOOL_ENTRIES: [embassy_rp::binary_info::EntryAddr; 4] = [
    embassy_rp::binary_info::rp_program_name!(c"uid-over-usb"),
